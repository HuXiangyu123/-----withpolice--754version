...
add_arg('resume_model',     str,   'models/CAMPPlus_MFCC/best_model/',  "模型的路径")
...
# 开始评估
start = time.time()
loss, accuracy ,recall  = trainer.evaluate(resume_model=args.resume_model,
                                  save_matrix_path=args.save_matrix_path)
end = time.time()
print('评估消耗时间：{}s，loss：{:.5f}，accuracy：{:.5f} , recall:{:5f}'.format(int(end - start), loss, accuracy,recall))
